...
const ProfilePage = () => {
  ...
  const profileBadges = [roles[0] || "customer", customer?.segment_type]
    .filter(Boolean)
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        const nextRoles = Array.isArray(currentUser.roles)
          ...
          : currentUser.role_id
            ? [
                roleLabelMap[currentUser.role_id] ||
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      message.success(response.data?.message || "Cập nhật hồ sơ thành công");
      ...
      <div className="mx-auto my-6 w-full max-w-[1280px] px-4 sm:my-10 sm:px-6 lg:px-8">
        <div className="mb-12 scroll-mt-24 text-center">
          ...
          <p className="mx-auto max-w-2xl text-lg text-gray-500">
            Cập nhật thông tin cá nhân và đồng bộ hồ sơ mua sắm của bạn
          </p>
        </div>

        <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-card sm:p-7">
          <div className="mb-6 flex flex-wrap items-center justify-end gap-2 border-b border-gray-100 pb-6">
            {profileBadges.map((badge) => (
              <span
                key={badge}
                className="rounded-md border border-[#EBCB79] bg-[#FFF9EA] px-4 py-1.5 text-2xl font-medium text-[#C58C46]"
              >
                {badge}
              </span>
            ))}
